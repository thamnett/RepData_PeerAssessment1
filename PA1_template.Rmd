...
mean_steps <- mean(step_day$dailysteps)
...
#calculate and print the interval with the max average steps
...
mean_fsteps <- mean(filled_day$dailysteps)

med_fsteps <- median(filled_day$dailysteps)
...
week_steps <- filled %>% 
  mutate(daytype = ifelse(weekdays(as.Date(as.character(date))) == "Saturday" |
                          ...
                          "weekend","weekday")) %>% 
  group_by(daytype) %>% summarize(dailysteps = sum(steps))

ggplot(data = week_steps, aes(x=interval, y=dailysteps)) + 
